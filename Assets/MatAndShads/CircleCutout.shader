Shader "OwnShaders/CircleCutout"
{
    Properties
    {
        ...
        // Arc cutout properties
        _StartAngle("Start Angle (deg)", Float) = 0
        _EndAngle("End Angle (deg)", Float) = 360
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // Arc cutout logic - remap UVs to sprite-local coordinates
                float2 localUV = (i.uv - _UVRemap.xy) / _UVRemap.zw;
                float2 centerUV = float2(0.5, 0.5);
                float2 offset = localUV - centerUV;
                float angle = atan2(offset.y, offset.x); // -PI to PI
                angle = angle < 0 ? angle + 2 * 3.14159265 : angle;
                float startRad = radians(_StartAngle);
                float endRad = radians(_EndAngle);
