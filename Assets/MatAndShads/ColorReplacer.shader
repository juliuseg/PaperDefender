Shader "OwnShaders/ColorReplacer"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 CombinedShapeLightFragment(Varyings i) : SV_Target
            {
                ...
                half3 colorDiff = abs(main.rgb - _SourceColor.rgb);
                half maxDiff = max(max(colorDiff.r, colorDiff.g), colorDiff.b);
                
                if (maxDiff <= _ColorTolerance)
                {
                    // Replace color while keeping original alpha and applying sprite renderer color
                    finalColor.rgb = _TargetColor.rgb * i.color.rgb;
                    finalColor.a = main.a * _AlphaMultiplier; // Apply alpha multiplier
                }
        ...
        Pass
        {
            ...
            float4 UnlitFragment(Varyings i) : SV_Target
            {
                ...
                half3 colorDiff = abs(mainTex.rgb - _SourceColor.rgb);
                half maxDiff = max(max(colorDiff.r, colorDiff.g), colorDiff.b);
                ...
                if (maxDiff <= _ColorTolerance)
                {
                    // Replace color while keeping original alpha and applying sprite renderer color
                    finalColor.rgb = _TargetColor.rgb * i.color.rgb;
                    finalColor.a = mainTex.a * _AlphaMultiplier; // Apply alpha multiplier
                }
